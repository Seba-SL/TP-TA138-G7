...
La tensión de referencia $v_{ref}$ deberá ser lo más baja posible para asegurar una tensión $V_{DO}$ lo más chica posible. Esto se debe a que, para una $V_{th} = \SI{4}{V}$ y una tensión de alimentación $V_{cc} \sim \SI{5}{V}$, se corre el riesgo de subir demasiado la tensión $vce_5 = v_{reg} - vbe_{on}$ por encima de $V_{cc} - V_{th}$, saturando el par diferencial. Para una tensión de referencia de $V_{reg-t} = \SI{2.5}{V}$, la tensión $vce_5 \approx \SI{1.8}{V}$. Esto limita nuevamente la tensión $V_{DO} = (vce_5 + V_{th}) - V_reg = \SI{800}{mV}$.

Se fija $R_r$ en un valor estándar que aporte una corriente de operación normal $\sim \SI{1}{mA}$. En este caso $R_r = \SI{1}{\kilo\ohm}$, esto da una corriente de operación de \SI{1,8}{mA} para una tensión de referencia $V_{ref-t} = \SI{2,5}{V}$, lo que es \SI{900}{mA} para cada transistor del par diferencial.
...
\begin{tikzpicture}[transform shape]
	...
	\draw (12, 11) -- (13, 11);
	\node[circ](N3) at (13, 11){} node[anchor=west] at (N3.text){$V_{ref\_t}$};
	\node[circ] at (12, 11){};
...
El transistor $T_1$ sirve a la función de entregar corriente, manteniendo la tensión de referencia fija. Este elemento es fundamental para alimentar los pares diferenciales de las figuras \ref{fig:lazo_tension2} y \ref{}.

De la hoja de datos, se obtiene la siguiente fórmula para esta topología:

\begin{equation}
V_{ref-t} = \SI{2.5}{V} \cdot \left( 1 + \frac{R_1}{R_2}\right)
\end{equation}
...
\begin{equation}
I_{TL431} = \frac{V_{cc}-(V_{ref-t} + vbe_{on})}{R_s},
\end{equation}
...
La elección de resistores se hace en función al compromiso $V_{DO}$-Efectividad del lazo. En este caso, se asigna $R_{s} = \SI{0.2}{\ohm}$. Para una corriente máxima, generará una caída de tensión de $V_{Rs} = \SI{0.3}{V}$, que mantiene el margen visto en la sección \ref{sec:lazo_tension} de $V_{DO} = \SI{0.8}{V}$.
...
Para solucionar este problema, se agrega un nuevo divisor resistivo a la entrada del amplificador. Como se quiere tener control sobre el valor final de $V_{Rs}$, se realimenta para obtener 

\begin{equation}
	v_f = (I_o \cdot R_s) \cdot \frac{R_2}{R_1}
\end{equation}

Se busca una tensión $v_f$ para corriente máxima $v_{fmax} < V_{reg}$. A tal efecto se asignan $(R_2 = \SI{10}{k\ohm},R_1 = \SI{1}{k\ohm}) \rightarrow v_{fmax} = \SI{3}{V}$.
...
Para el diseño amplificador final, se utiliza la misma topología del lazo de corriente, de la figura \ref{fig:lazo_tension2}.
...
\begin{equation}
	v_{ref-c} = \alpha v_{ref-t} + \beta v_o.
\end{equation}
Comportamiento que puede obtenerse fácilmente con un circuito amplificador sumador, como el de la figura \ref{fig:vref_c}.
...
$$v_{ref-c} = \alpha v_{ref-t} = v_{fmin} = I_{FBCK} \cdot R_s \cdot \frac{R2}{R1} \rightarrow \SI{0.8}{V},$$
entonces, 

\begin{equation}
\alpha = 0.32 = \frac{R_{\alpha2}}{R_{\alpha1} + R_{\alpha2}}.
\end{equation}

Del mismo modo, para la corriente máxima, se esperará que $v_o = V_{reg}$. Como tal

$$v_{ref-c} = \alpha v_{ref-t} + \beta v_o = v_{fmax} = I_{max} \cdot R_s \cdot \frac{R2}{R1} \rightarrow \SI{3}{V},$$
que define
\begin{equation}
\beta = 0.44 = \frac{R_{\beta2}}{R_{\beta1} + R_{\beta2}}.
\end{equation}
...
\begin{tikzpicture}[transform shape]
	...
	\draw (8, 8) -- (10, 8);
	\node[circ](N1) at (5.38, 12.01){} node[anchor=east] at (N1.text){$v_{ref-t}$};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[transform shape]
		...
		\node[shape=rectangle, minimum width=1.27cm, minimum height=1cm] at (-20.5, 10.5){} node[anchor= south west, align=left, text width=0.917cm, inner sep=5pt] at (-21.135, 11){$V_{ref-t}$};
		...
		\node[shape=rectangle, minimum width=1.27cm, minimum height=1cm] at (-20.385, 7.75){} node[anchor= south west, align=left, text width=0.917cm, inner sep=5pt] at (-21.02, 8.25){$V_{ref-c}$};
...
\end{figure}
	...
	\begin{equation}
		I{c1/4} \approx \frac{V_{cc}-vbe_{on}}{R_1},
	\end{equation}
	...
	Lo mismo ocurre para los resistores $R_{2/4}$, pero en este caso se busca una corriente de $\sim \SI{2}{mA}$. Nuevamente, para valores de control $v_{control} \approx \SI{5}{V}$, se obtiene 
	\begin{equation}
	I_{R4} \approx \SI{2}{mA} \rightarrow R_4 = \SI{2.2}{k\ohm}
	\end{equation}
	y
	\begin{equation}
	I_{R2} \approx \SI{2}{mA} = \frac{V_{cc} - (\SI{5}{V} + vbe_{on})}{R_2} \rightarrow R_2 = \SI{2.2}{k\ohm}
	\end{equation}


	\subsubsection{Simulaciones - Modo estático}
	...
	Para caracterizar este comportamiento, se realiza un barrido de la tensión de entrada y se gráfica la tensión de salida \(V_O\) en función de \(V_{CC}\).
	...
	\end{itemize}
		...
		En la figura \ref{fig:ldoregulacionlinea} puede verse la curva de salida en función de la entrada para $\SI{0}{V} < V_{cc} < \SI{4}{V}$. De la simulación se obtiene
		
		\begin{equation}
			Regulacion \ de \ Linea = 260,67 \frac{\si{\micro V}}{\si{V}},
		\end{equation}
		
		con un pequeño offset $V_{offset} = \SI{20}{mV}$, resultante de $V_ref-t = \SI{2.49}{V} \ne \SI{2.5}{V}$.

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\linewidth]{"../../Simulación/LDO/Graficos en Python/Capturas/LDO_regulacion_linea"}
		\caption{Gráfico de los datos de simulación de linea,}
	...
	La regulación de carga puede analizarse evaluando la variación de la tensión de salida \(V_O\) frente a cambios en la resistencia de carga \(R_L\).
	...
	En la figura \ref{fig:ldoregulacioncarga} puede verse la gráfica resultante. Notar que, al poseer un lazo de corriente, modificar la corriente de entrada directamente puede dar resultados inexactos, como valores de tensión $V_o$ negativos. Aun así, para la zona de control propia del lazo de tensión, pueden obtenerse resultados:
	
	\begin{equation}
		\frac{\Delta V}{\Delta I} = -931,7 \frac{\si{\micro V}}{\si{A}} = R_T
	\end{equation}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\linewidth]{"../../Simulación/LDO/Graficos en Python/Capturas/V vs I"}
	...
	Para medir la eficiencia del circuito, se ingresan diferentes valores de tensión de entrada $V_{cc}$ y se evalúa la potencia de entregada y la potencia sobre la carga. El resultado se observa en la figuar \ref{fig:ldoeficiencia}.
	...
	Como tal, se espera una eficiencia máxima para valores $V_{cc}$ cercanos a $V_{reg}$ y resistencias de carga $R_L$ altas.
	...
	\begin{itemize}
		...
		\item \textbf{Transistor de shunt:} Debido a que el lazo de corriente es lento, es necesario implementar un transistor de shunt que limite la corriente de forma rápida ante repentinos cambios de carga. Debido a que el transistor de paso es canal P, su implementación no es tan directa.
		
		\item \textbf{$V_{DO}$:} Aunque se obtiene una tensión $V_{DO} \approx \SI{0.8}{V}$, puede reducirse a valores cercanos a \SI{0.3}{V} si se cambia la tensión de referencia por un valor menor. Eso implica cambiar el TL431 por algún par como el \textbf{TLV431} con una tensión de referencia mínima menor, y el cambio de resistores consecuente.
